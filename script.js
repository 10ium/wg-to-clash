...
function validateAndComplete(config, source) {
    ...
    if (config.name) {
        ...
        config.name = `${emoji} ${identifier}`.trim().replace(/\s+/g, ' ');
    } else {
...
function parseAllInputs(textContent) {
    ...
    try { structuredConfig = jsyaml.load(textContent); } catch (e) { return parseFromText(textContent); }
    if (typeof structuredConfig === 'object' && structuredConfig !== null) {
        if (structuredConfig.outbounds) return parseFromSingBox(structuredConfig);
        if (structuredConfig.proxies) return parseFromMihomo(structuredConfig);
    }
...
processInputBtn.addEventListener('click', async function handleProcessInput() {
    ...
    const parsedResults = parseAllInputs(allRawText);
    ...
    showMessage(`انجام شد! (${successfulConfigs.length} کانفیگ اضافه شد، ${errorDetails.length} خطا)`, 'success');
    displayErrorDetails(errorDetails);
    wgConfigInput.value = '';
    wgConfigFile.value = '';
    uploadedFilesContent = [];
...
generateBtn.addEventListener('click', async function handleGenerateAndDownload() {
    ...
    try {
        ...
        const outputFileName = `Mihomo_WG_[${templateName}].yaml`;
        downloadFile(outputFileName, finalYaml);
        showMessage(`فایل با موفقیت تولید و دانلود شد!`, 'success');
