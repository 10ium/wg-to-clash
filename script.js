...
/**
 * Post-processes the generated YAML string to ensure problematic strings are quoted and placeholders are correctly filled.
 * @param {string} yamlString - The raw YAML string.
 * @param {Array<string>} proxyNames - Array of generated proxy names.
 * @returns {string} The fixed YAML string with necessary quotes and proxy lists.
 */
function fixYamlQuotingAndPlaceholders(yamlString, proxyNames) {
    ...
    const proxyListIndent = '      '; // Indentation for proxy list items under 'proxies:' in proxy-groups
    const formattedProxyNames = proxyNames.map(name => `${proxyListIndent}- "${name}"`).join('\n');

    // Replace PROXIES_PLACEHOLDER
    fixedYaml = fixedYaml.replace(/\s*# PROXIES_PLACEHOLDER #/, (match) => {
        // We'll replace this with the actual proxy configurations, which are generated by jsyaml.dump
        // This placeholder is more for conceptual clarity in the template, as jsyaml.dump fills 'proxies' directly.
        // If you intended to dynamically insert *raw text* here, the logic would be different.
        // Assuming jsyaml.dump handles the 'proxies' section directly, this placeholder mainly serves as a marker for where the list begins.
        return ''; // jsyaml.dump already generates the proxies list. This line will be effectively removed.
    });


    // Replace MANUAL_PROXIES_LIST_PLACEHOLDER
    fixedYaml = fixedYaml.replace(/\s*# MANUAL_PROXIES_LIST_PLACEHOLDER #/, formattedProxyNames);

    // Replace AUTO_PROXIES_LIST_PLACEHOLDER
    fixedYaml = fixedYaml.replace(/\s*# AUTO_PROXIES_LIST_PLACEHOLDER #/, formattedProxyNames);
...
async function handleGenerate() {
    ...
    // The proxy-groups will be handled in post-processing by replacing placeholders
    // We explicitly remove proxy lists from the JS object to ensure they are empty
    // before dumping, allowing the `fixYamlQuotingAndPlaceholders` to insert them cleanly.
    if (mihomoConfig['proxy-groups'] && Array.isArray(mihomoConfig['proxy-groups'])) {
        mihomoConfig['proxy-groups'].forEach(group => {
            if (group.name === "دستی 🤏🏻" || group.name === "خودکار (بهترین پینگ) 🤖") {
                // Set these to an empty array so jsyaml.dump generates 'proxies: []' or just 'proxies:'
                // which our placeholder replacement can then target.
                group.proxies = []; 
            }
        });
    }
    ...
    try {
        ...
        // Apply post-processing to fix quoting issues and insert proxy names into proxy-groups
        yamlString = fixYamlQuotingAndPlaceholders(yamlString, proxyNames);
